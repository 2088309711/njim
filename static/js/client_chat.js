!function () {
    ...
    var show_msg = new Vue({
        ...
        methods: {
            add: function (ojb) {
                ...
                //转义消息内容
                ojb.content = replaceMsgContent(ojb.content, 'get');
    ...
    function sendMsg(content) {
        ...
        // 获取信息
        var content = content != '' ? content : getInputText();
        ...
        var rContent = replaceMsgContent(content, 'send');// 替换信息内容
    ...
    // 替换消息内容
    function replaceMsgContent(content, type) {
        var regArr;
        if (type === 'get') {
            regArr = [
                ['\\[#', '<span class="msg-anchor-point" onclick="sendAsk(this)">'],
                ['#]', '</span>']
            ]
        } else if (type === 'send') {
            regArr = [
